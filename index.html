...
<script>
    ...
    function convertShade(shade) {
        let upper = shade.trim().toUpperCase();
        return shadeMap[upper] || (upper ? `Not found (${upper})` : "");
    }
    ...
    function displayShade() {
        ...
        let convertedIncisal = convertShade(incisal);
        let convertedBody = convertShade(body);
        let convertedGingival = convertShade(gingival);
